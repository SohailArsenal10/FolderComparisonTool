package com.javafolderapp.folderComparisonToolV1;
...
import com.javafolderapp.folderComparisonToolV1.service.SheetsServiceUtil;
...
public class GoogleSheetsLiveTest {
    ...
    private static final String SPREADSHEET_ID = "1PtVSHNNvt3OZCRwDx9adVXCoovciCOGDxSE_KxkGWtE";
    ...
    public void whenWriteSheet_thenReadSheetOk() throws IOException {
        ValueRange body = new ValueRange().setValues(Arrays.asList(Arrays.asList("Expenses January"), Arrays.asList("books", "40"), Arrays.asList("pens", "20"), Arrays.asList("Expenses February"), Arrays.asList("clothes", "30"), Arrays.asList("shoes", "6")));
        ...
        data.add(new ValueRange().setRange("D1").setValues(Arrays.asList(Arrays.asList("January Total New", "=B2+B3"))));
        data.add(new ValueRange().setRange("D4").setValues(Arrays.asList(Arrays.asList("February Total New", "=B5+B6"))));
        ...
        assertThat(januaryTotal.getValues().get(0).get(0)).isEqualTo("60");

        ValueRange febTotal = readResult.getValueRanges().get(1);
        assertThat(febTotal.getValues().get(0).get(0)).isEqualTo("35");
